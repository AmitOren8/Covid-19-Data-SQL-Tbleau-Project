...
select 
	...
	total_cases, 
	cast(total_deaths as int), -- In original data the total_deaths column is type nvarchar.
	(cast(total_deaths as int)/total_cases)*100 as precentage_death_of_cases
from CovidDeaths$
where continent is not null
--where location = 'israel'
order by 1,2

-- 2.1.2. Likelihood of dying if contracted with covid-19 in every country.
select 
	location,  
...
-- 2.1.3. North Korea shows non-logical resultes, let's review North Korea's data.
...
select 
	...
	population, 
	total_cases, 
	(total_cases/population)*100 as precentage_cases
from CovidDeaths$
where continent is not null
--where location = 'israel'
order by 1, 2 

-- 2.2.1. Precentage of the population that contracted covid-19 per country.
select 
	location, 
	population, 
	max(total_cases) as total_infected, 
...
select 
	...
	(total_cases/population)*100 as precentage_deaths
from CovidDeaths$
where continent is not null
--where location = 'israel'
order by 1, 2 

-- 2.3.1. Precentage of the population that died of covid-19 per country.
select 
	location, 
	population, 
	max(cast(total_deaths as int)) as total_deceased, 
	max((total_deaths/population))*100 as precentage_deceased
from CovidDeaths$
where continent is not null
--where location = 'israel'
group by location, population
...
-- 5.1 Total global numbers
select
	location,
	population,
	cases_overall,
	round((cases_overall / population)*100, 5) as precentage_infected,
	dead_over_all,
	round((dead_over_all / population)*100, 5) as precentage_dead
from(select
		location,
		population,
		max(total_cases) as cases_overall,
		max(cast(total_deaths as float)) as dead_over_all
	from CovidDeaths$
	where location = 'world'
	group by location, population) as a


-- 5.2. Global Covid-19 cases & deaths growth rates

-- 5.2.1.  Global Covid-19 cases growth rates
...
-- 5.2.2. Global Covid-19 deaths growth rates
...
select
	...
	sum_new_deaths,
	prev_new_deaths,
	round((new_deaths / prev_new_deaths)*100, 3) as deaths_precentage_growth
...
select *, (sum_vaccinations / population)*100 as percent_vaccinated
